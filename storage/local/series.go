...
// new series).
func newMemorySeries(m model.Metric, chunkDescs []*chunk.ChunkDesc, modTime time.Time) (*memorySeries, error) {
	var err error
...
func (s *memorySeries) Add(v model.SamplePair) (int, error) {
	if len(s.chunkDescs) == 0 || s.headChunkClosed {
		newHead := chunk.NewChunkDesc(chunk.NewChunk(), v.Timestamp)
	...
	for _, c := range chunks[1:] {
		s.chunkDescs = append(s.chunkDescs, chunk.NewChunkDesc(c, c.FirstTime()))
	}
...
func (s *memorySeries) evictChunkDescs(iOldestNotEvicted int) {
	...
	if lenToKeep < len(s.chunkDescs) {
		...
		s.chunkDescs = append(
			make([]*chunk.ChunkDesc, 0, lenToKeep),
...
func (s *memorySeries) dropChunks(t model.Time) error {
	...
	s.chunkDescs = append(
		make([]*chunk.ChunkDesc, 0, len(s.chunkDescs)-keepIdx),
...
func (s *memorySeries) preloadChunks(
	...
	loadIndexes := []int{}
	pinnedChunkDescs := make([]*chunk.ChunkDesc, 0, len(indexes))
...
func (s *memorySeries) NewIterator(
	pinnedChunkDescs []*chunk.ChunkDesc,
	...
	return &memorySeriesIterator{
		chunks:           chunks,
		chunkIts:         make([]chunk.ChunkIterator, len(chunks)),
...
func (s *memorySeries) head() *chunk.ChunkDesc {
...
//
// The caller must have locked the fingerprint of the series.
func (s *memorySeries) chunksToPersist() []*chunk.ChunkDesc {
...
type memorySeriesIterator struct {
	...
	chunkIt chunk.ChunkIterator
	// Caches chunkIterators.
	chunkIts []chunk.ChunkIterator
...
func (it *memorySeriesIterator) chunkIterator(i int) chunk.ChunkIterator {
